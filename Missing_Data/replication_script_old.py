...
try:
    with Popen(
        ["./missing", "-f", "test.cfg", 
         "--info_file", "Rect10x10/info.dat", 
         "--neighbors_file", "Rect10x10/neighbors.dat",
         "--arrivals_file", "Rect10x10/calls.dat",
         "--missing_file", "Rect10x10/missing_calls.dat"
         "--mn_samples_file", "Rect10x10/mn_samples_file.dat"],
        bufsize=1,
        stdout=PIPE,
        stderr=PIPE,
        text=True,
...
R_hex = 226
mn_samples = np.zeros((C, D, T, max_obs, S, R_hex))
output_file = f"Hex7_km/mn_samples.dat"
arq = open(output_file, "w")
for c in range(C):
    for d in range(D):
        print("Generating multinomial samples")
...
try:
    with Popen(
        ["./missing", "-f", "test.cfg", 
         "--info_file", "Hex7_km/info.dat", 
         "--neighbors_file", "Hex7_km/neighbors.dat",
         "--arrivals_file", "Hex7_km/calls.dat",
         "--missing_file", "Rect10x10/missing_calls.dat"
         "--mn_samples_file", "Hex7_km/mn_samples_file.dat"],
        bufsize=1,
        stdout=PIPE,
        stderr=PIPE,
        text=True,
...
class Stats(object):
    def __init__(self, lam):
        ...
        for t in range(T):
            ...
            self.q05_total[t] = stats.poisson.ppf(0.05, self.mean_total)
            self.q95_total[t] = stats.poisson.ppf(0.95, self.mean_total)
            self.mean_p0[t] = np.sum(lam[0, :, t])
            self.q05_p0[t] = stats.poisson.ppf(0.05, self.mean_p0)
            self.q95_p0[t] = stats.poisson.ppf(0.95, self.mean_p0)
            self.mean_p1[t] = np.sum(lam[1, :, t])
            self.q05_p1[t] = stats.poisson.ppf(0.05, self.mean_p1)
            self.q95_p1[t] = stats.poisson.ppf(0.95, self.mean_p1)
            self.mean_p2[t] = np.sum(lam[2, :, t])
            self.q05_p2[t] = stats.poisson.ppf(0.05, self.mean_p2)
            self.q95_p2[t] = stats.poisson.ppf(0.95, self.mean_p2)
